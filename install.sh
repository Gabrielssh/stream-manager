...
# =====================================
# IPTV PRO SERVER + BOT TELEGRAM INSTALL
# =====================================
...
apt update -y
apt install -y ffmpeg nginx curl vnstat python3 python3-pip glances
...
select_quality(){
  ...
  echo "1) 360p"; echo "2) 480p"; echo "3) 720p"; echo "4) 1080p"; echo "5) Melhor disponível"
  read -rp "Opção: " Q
  case "$Q" in
...
add_channel(){ clear
  ...
  echo "Canal iniciado"; pause
}

export_links_backup(){ DATE=$(date +%Y%m%d_%H%M%S)
  cp "$DB" "$BACKUP_DIR/links_backup_$DATE.db"
  echo "Backup criado: $BACKUP_DIR/links_backup_$DATE.db"; pause
}

import_links_backup(){
  echo "Backups disponíveis:"; ls -1 "$BACKUP_DIR"/links_backup_*.db 2>/dev/null || echo "Nenhum backup"
  read -rp "Nome do arquivo: " FILE
  FULL_PATH="$BACKUP_DIR/$FILE"
  if [ -f "$FULL_PATH" ]; then cp "$FULL_PATH" "$DB"
    while IFS="|" read -r NAME LINK QUALITY; do create_service "$NAME" "$LINK" "$QUALITY"; done < "$DB"
    echo "Backup importado!"; else echo "Arquivo não encontrado!"; fi
  pause
}
...
delete_channel(){ echo "Canais:"; cut -d "|" -f1 "$DB"; read -rp "Nome canal: " NAME; NAME=$(sanitize "$NAME")
if grep -q "^$NAME|" "$DB"; then systemctl stop iptv-$NAME 2>/dev/null; systemctl disable iptv-$NAME 2>/dev/null; rm -f /etc/systemd/system/iptv-$NAME.service "$HLS/$NAME.m3u8" "$HLS/$NAME"*.ts "/root/iptv_pro/run-$NAME.sh"; sed -i "/^$NAME|/d" "$DB"; systemctl daemon-reload; echo "Canal $NAME excluído"; else echo "Canal não encontrado"; fi; pause; }
auto_clean_segments(){ read -rp "Intervalo (minutos): " INTERVAL; INTERVAL=${INTERVAL:-5}; while true; do find "$HLS" -name "*.ts" -mmin +"$INTERVAL" -delete; sleep "$((INTERVAL*60))"; done; }
...
export_playlist(){ IP=$(hostname -I | awk '{print $1}'); echo "#EXTM3U" > "$PLAYLIST"; while IFS="|" read -r NAME LINK QUALITY; do echo "#EXTINF:-1,$NAME" >> "$PLAYLIST"; echo "http://$IP:8080/$NAME.m3u8" >> "$PLAYLIST"; done < "$DB"; pause; }
clean_segments(){ find "$HLS" -name "*.ts" -mmin +10 -delete; pause; }
backup(){ tar -czf "$BASE/backup/iptv_full_backup.tar.gz" "$BASE"; echo "Backup completo"; pause; }
restart_hls(){ systemctl restart nginx; pause; }
show_off(){ while IFS="|" read -r NAME LINK QUALITY; do if ! systemctl is-active --quiet iptv-$NAME; then echo "$NAME OFF"; fi; done < "$DB"; pause; }
show_uptime(){ while IFS="|" read -r NAME LINK QUALITY; do echo "$NAME"; systemctl show iptv-$NAME --property=ActiveEnterTimestamp; echo; done < "$DB"; pause; }
...
start_bot_telegram(){
  BOT="$BASE/iptv_bot.py"
  if [ ! -f "$BOT" ]; then
    cat > "$BOT" <<'EOPY'
#!/usr/bin/env python3
# Coloque aqui o código do bot que você forneceu
EOPY
    chmod +x "$BOT"
    cat > /etc/systemd/system/iptv_bot.service <<EOF
[Unit]
...
EOF
    systemctl daemon-reload
    systemctl enable iptv_bot
    systemctl start iptv_bot
    echo "Bot Telegram iniciado"
  else
    systemctl restart iptv_bot
    echo "Bot Telegram reiniciado"
  fi
  pause
}
...
echo "21) Iniciar/Reiniciar Bot Telegram"
echo "0) Sair"
read -rp "Opção: " OP
case "$OP" in
1) add_channel ;;
